...
export default async function AdminConfigPage() {
  ...
  const window = await serverFetch<AdminRecommendationWindow>('/api/admin/recommendation-window');

  if (!cfg.ok || !pm.ok || !sched.ok || !window.ok) {
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted">기준값 · 프로모션 금액 · 추출 운영 시간대 · 추천 시간 윈도우</p>
      </header>
      <ConfigForm
        initial={cfg.data}
        initialPromotionMap={pm.data}
        initialSchedule={sched.data}
